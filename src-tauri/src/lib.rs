...
/// Check entitlement status via StoreKit 2 (macOS)
#[cfg(target_os = "macos")]
#[tauri::command]
async fn check_entitlement() -> Result<String, String> {
    let swift_script = r#"
import StoreKit
import Foundation

@available(macOS 12.0, *)
func checkEntitlement() async -> String {
    ...
    for await entitlement in Transaction.currentEntitlements {
        if case .verified(let transaction) = entitlement {
            if transaction.productID.contains("premium") {
                ...
                result["purchaseDate"] = formatter.string(from: transaction.purchaseDate)
...
if #available(macOS 12.0, *) {
    ...
    var output = ""
    Task {
        output = await checkEntitlement()
        semaphore.signal()
    }
    semaphore.wait()
    print(output)
} else {
    print("{\"isPremium\":false,\"purchaseDate\":null,\"productId\":null,\"source\":\"storekit\"}")
}
"#;
    ...
    let output = Command::new("swift")
        ...
        .arg(swift_script)
        .output()
        .map_err(|_| "Failed to check entitlement".to_string())?;

    if output.status.success() {
        Ok(String::from_utf8_lossy(&output.stdout).trim().to_string())
    } else {
        Ok("{\"isPremium\":false,\"purchaseDate\":null,\"productId\":null,\"source\":\"storekit\"}".to_string())
    }
}

/// Purchase premium via StoreKit 2 (macOS)
#[cfg(target_os = "macos")]
#[tauri::command]
async fn purchase_premium() -> Result<String, String> {
    let swift_script = r#"
import StoreKit
import Foundation
    ...
    do {
        ...
        case .success(let verification):
            if case .verified(let transaction) = verification {
                ...
                let formatter = ISO8601DateFormatter()
                let dateStr = formatter.string(from: transaction.purchaseDate)
                return "{\"isPremium\":true,\"purchaseDate\":\"\(dateStr)\",\"productId\":\"\(transaction.productID)\",\"source\":\"storekit\"}"
            }
            return "{\"isPremium\":false,\"purchaseDate\":null,\"productId\":null,\"source\":\"storekit\"}"
        case .userCancelled:
            return "{\"isPremium\":false,\"purchaseDate\":null,\"productId\":null,\"source\":\"storekit\"}"
...
"#;
    ...
    if output.status.success() {
        ...
        Err("Purchase failed".to_string())
    }
}

/// Restore purchases via StoreKit 2 (macOS)
#[cfg(target_os = "macos")]
#[tauri::command]
async fn restore_purchases() -> Result<String, String> {
    let swift_script = r#"
import StoreKit
import Foundation
...
"#;
    ...
    let output = Command::new("swift")
        ...
        .arg(swift_script)
        .output()
        .map_err(|_| "Failed to restore purchases".to_string())?;
...
/// Check entitlement via Windows Store API
#[cfg(target_os = "windows")]
#[tauri::command]
async fn check_entitlement() -> Result<String, String> {
    ...
    let context = StoreContext::GetDefault()
        .map_err(|e| format!("Failed to get store context: {}", e))?;

    let license = context.GetAppLicenseAsync()
        .map_err(|e| format!("Failed to get license: {}", e))?
        .get()
        .map_err(|e| format!("Failed to get license result: {}", e))?;

    let add_on_licenses = license.AddOnLicenses()
        .map_err(|e| format!("Failed to get add-on licenses: {}", e))?;

    if let Ok(premium_license) = add_on_licenses.Lookup(&windows::core::HSTRING::from("com.qliplab.premium")) {
        if premium_license.IsActive().unwrap_or(false) {
            return Ok("{\"isPremium\":true,\"purchaseDate\":null,\"productId\":\"com.qliplab.premium\",\"source\":\"windows_store\"}".to_string());
        }
    }

    Ok("{\"isPremium\":false,\"purchaseDate\":null,\"productId\":null,\"source\":\"windows_store\"}".to_string())
}

/// Purchase premium via Windows Store
#[cfg(target_os = "windows")]
#[tauri::command]
async fn purchase_premium() -> Result<String, String> {
    use windows::Services::Store::StoreContext;
    ...
    let result = context.RequestPurchaseAsync(&windows::core::HSTRING::from("com.qliplab.premium"))
    ...
    if status == windows::Services::Store::StorePurchaseStatus::Succeeded ||
       status == windows::Services::Store::StorePurchaseStatus::AlreadyPurchased {
        Ok("{\"isPremium\":true,\"purchaseDate\":null,\"productId\":\"com.qliplab.premium\",\"source\":\"windows_store\"}".to_string())
    } else {
        Ok("{\"isPremium\":false,\"purchaseDate\":null,\"productId\":null,\"source\":\"windows_store\"}".to_string())
    }
}

/// Restore purchases via Windows Store
#[cfg(target_os = "windows")]
#[tauri::command]
async fn restore_purchases() -> Result<String, String> {
    // Windows Store handles license automatically via check_entitlement
    check_entitlement().await
}

/// IAP fallback for Linux
#[cfg(target_os = "linux")]
#[tauri::command]
async fn check_entitlement() -> Result<String, String> {
    Ok("{\"isPremium\":false,\"purchaseDate\":null,\"productId\":null,\"source\":null}".to_string())
}

#[cfg(target_os = "linux")]
#[tauri::command]
async fn purchase_premium() -> Result<String, String> {
    Err("In-app purchases are not available on Linux".to_string())
}
